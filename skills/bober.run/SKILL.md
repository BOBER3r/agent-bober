...
3. Log event:
   ...
   ```
4. **Spawn the Documenter subagent — write docs now, per-sprint, while the change is fresh.** Do NOT defer documentation to a final sprint. Read `documenter.docsMode` from `bober.config.json` (default `committed`, already in context from step 3g) and `documenter.docsDir` (optional). Resolve the record path: `committed` (no `docsDir`) → repo-relative `docs/sprints/<contractId>.md`, committed by the documenter after also updating related stale docs (README, ADRs, CLAUDE.md, module docs); `local` (no `docsDir`) → the same relative path resolved to an absolute path under the project root, no commit, no other repo file touched, stale docs go to `concerns` (a deterministic pre-step already gitignores the directory); `external` → `~/.bober/docs/<project.name>/sprints/<contractId>.md`, outside the repo, same no-commit/no-other-edits behavior, no git operation at all. `docsDir`, when set, overrides the default location in every mode (relative → joined to project root, absolute/`~`-prefixed → used as-is). Use the Agent tool with `subagent_type: bober-documenter` and `mode: auto`, interpolating the resolved path as `<resolved record path>`:
   ```
   ...
   Follow your agent instructions: determine what was built from the committed diff and write the sprint record to <resolved record path>. In `committed` mode, also find & update related existing docs (README, ADRs, CLAUDE.md, module docs) made stale by the change, and commit ONLY the doc files separately. In `local`/`external` mode, do NOT modify any other repo file and do NOT stage or commit anything — report stale docs in "concerns" instead. Do NOT modify application code or tests. Respond with the JSON structure defined in your agent spec.
   ```
