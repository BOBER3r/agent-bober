...
| `pipeline.engine` | `'ts'\|'skill'\|'workflow'\|'medical-sop'\|'pge'` | `'ts'` | 0.16.0 | Orchestration engine selection behind the engine-selection seam. `'ts'` runs the built-in TypeScript pipeline (default). `'skill'` and `'workflow'` select alternative engines; an eligibility probe downgrades `'workflow'` → `'ts'` when ineligible or in `careful` mode. `'pge'` runs the graph engine (superstep interpreter over a serializable topology) and is an explicit opt-in that **never becomes the default** — an uncompilable topology downgrades to `'ts'` inside `PgeEngine.run`. No behavior change on the default `'ts'` path. |
