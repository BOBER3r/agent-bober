...
## Configurable documenter docs output (solo vs team) — 2 of 3 in progress
...
with solo-vs-team recipes) and a CHANGELOG entry land in the same sprint. Still pending: the
`bober init` solo-vs-team question — **sprint 3**; until it lands, a non-default `docsMode` is a
manual `bober.config.json` edit.
