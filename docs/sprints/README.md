...
## Obsidian Vault Storage Layer — in progress (2 of 5)
...
The full plan layers a `bober vault reindex` command (reusing the existing reconcile-at-ingest
path), an on-device-only Obsidian MCP read/write adapter, and a `profile.yaml` hook for later
SOPS wiring. **The derived FactStore index now exists** (Sprint 2): pure `noteToFacts` frontmatter→`FactInput`
mapping + `reindexNotes` that writes through the existing `writeFact` reconcile path (re-run = all
`noop`, changed value supersedes, `status:superseded` notes excluded) — still consumed in-process,
the `bober vault reindex` CLI / Obsidian MCP adapter / `profile.yaml` hook remain Sprints 3–5.
Sprint 1 lays the **foundation only** — a new `src/vault/` module with the typed
