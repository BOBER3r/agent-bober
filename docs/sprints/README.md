...
## Pluggable Code-Graph Backend (tokensave or code-review-graph) — in progress (1 of 8)
...
byte-unchanged, and the tokensave path is byte-identical. **No** `config.graph.backend` /
detection / `resolveGraphBackend` (Sprint 3), **no** transport/serveArgs/prereq/CLI change
(Sprint 2), and **no** code-review-graph code, tool names, or fixtures yet (Sprints 4–6). Full
suite **4974 passed | 1 skipped | 0 failed**; all 6 required + 1 optional criteria passed
iteration 1, zero regressions.
