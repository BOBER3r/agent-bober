...
golden executor pins one config precisely to exclude; **`critique` and `rework_route` await a
golden scenario** (a corrected-but-recorded-`fail` sprint alongside an otherwise passing run);
...
ADR-6; **`evaluatorFeedback` and `generatorNotes` need a PGE-node writer** neither this spec
...
mechanism** to execute under; and the flip bar itself — *"sustained green conformance across
