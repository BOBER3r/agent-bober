...
## PGE Real-Workload Viability + an Error Channel on `PipelineResult` — in progress (4 of 9)
...
git-effect `commit` node currently returns `success: true` because `PipelineResult` has no error
channel. Sprint 1 deliberately **fixes nothing**: it stops deriving and starts observing, driving a
...
refused reports `success: true`. Sprints 5 onward take that up.
