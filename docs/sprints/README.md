...
## PGE Real-Workload Viability + an Error Channel on `PipelineResult` — in progress (2 of 9)

`spec-20260812-pge-real-workload-errors` sets out to make `PgeEngine` able to execute a
*realistic* plan at all — every channel in `.bober/topology/coding.json` caps at 4,096 bytes while
this repository's own committed `PlanSpec` is an order of magnitude larger — and then to give the
