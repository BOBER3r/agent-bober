...
callback no `onTextDelta` reaches `chat()` and behavior is **byte-identical**, no new dep.
...
`parallelReadOnlyTools` tool-execution flag, and for the `ChatParams.onTextDelta` streaming callback
(Anthropic streams; other adapters no-op), lives in [`docs/providers.md`](../providers.md).
