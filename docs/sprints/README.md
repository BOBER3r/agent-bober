...
## Telegram Frontend — in progress (Sprint 5)
...
grammy stays `bot.ts`-only (new `BotTransport.downloadDocument` via `getFile`+`fetch`).
...
`sendKeyboard`/`answerCallback` transport extension), and (Sprint 5) the **document-upload medical
...
(`sendSafeKeyboard` as the sole keyboard chokepoint, closing the Sprint-4 seam gap) are documented in
