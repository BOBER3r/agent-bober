...
## Close every conformance divergence and run every graph node — in progress (2 of 11)
...
five checkpoint ids are permanently undeclarable, so `audits` joins `history` as recommended for
permanent acceptance, and the spec's own `feat-1`/`feat-3` were amended to say so rather than the
sprint passing quietly.**
