...
leaves open, deliberately and on the record: **`history` and `audits` are now recommended
for permanent acceptance**, not open work — there is no curator node to emit a start/complete
pair from, and five of the eight checkpoint ids the imperative pipeline records sit inside
...
acceptance is verified rather than merely unrevisited)*; **`evaluatorFeedback` and
...
real runs"* — is now **unsatisfiable by design**: two fields permanently accepted and two
that cannot close without a topology change mean `diffs` can never become empty under the
bar's current wording. Re-specifying that bar is the next spec's decision, not this one's.
