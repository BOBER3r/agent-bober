...
## Pluggable Code-Graph Backend (tokensave or code-review-graph) — in progress (2 of 8)
...
parsers move out of `prereq.ts`/`cli.ts` into `TokensaveBackend`. Still **no** `config.graph.backend`
/ detection / `resolveGraphBackend` (Sprint 3) and **no** code-review-graph code, tool names, or
fixtures yet (Sprints 4–6); the tokensave path stays byte-identical.
