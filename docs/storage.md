...
| **Sprint doc record** | `docs/sprints/<contractId>.md` (default) | Written by the per-sprint documenter (`src/orchestrator/documenter-agent.ts`) after a sprint's evaluator returns PASS. **Location and git behavior are configurable** as of spec-20260731 sprint 1 via `documenter.docsMode` (`committed` \| `local` \| `external`, default `committed`) and the optional `documenter.docsDir` override, resolved by `resolveSprintDocPath`: `committed` keeps the repo-relative `docs/sprints/<contractId>.md` and the agent commits it; `local` writes inside the repo but **never commits** (a deterministic `ensureGitignoreEntry` pre-step in `src/utils/git.ts` ensures `.gitignore` covers the directory); `external` writes to `~/.bober/docs/<project.name>/sprints/<contractId>.md` — **outside the repo**, with no git operation at all. `docsDir` (relative to the project root, or absolute / `~`-prefixed) overrides the default in every mode. The resolved path is echoed into the `history.jsonl` documentation event. |
