...
const parser = new ChatgptUiScrapeParser();

// ── sc-10-3: pure fn RawScrape -> ParsedAnswer{answerText, citations} ──────

describe("ChatgptUiScrapeParser — pure markdown -> {answerText, citations} (sc-10-3)", () => {
  it("extracts the answer prose and the markdown-link citations from a typical chatgpt-ui answer page", () => {
    const markdown = [
      "Target.example is one of the most popular casino review sites, known for",
      "in-depth reviews and fast payout guides.",
      "",
      "### Sources",
    ...
    const out = parser.parse({ url: "https://chatgpt.com/c/abc", markdown });
